...
/// <summary>
/// Data Access Layer for NguoiDung table
/// </summary>
public class NguoiDungDAL
{
    private readonly NganHangContext _context;

    public NguoiDungDAL(NganHangContext context)
    {
        _context = context;
    }
    ...
    public async Task<List<NguoiDungDTO>> GetDanhSachNguoiDungAsync(string? role, string? status)
    {
        ...
        {
            whereClause += " AND nd.VaiTro = @VaiTro";
        }
        if (!string.IsNullOrEmpty(status))
        {
            whereClause += " AND nd.TrangThai = @TrangThai";
        }
        ...
        var query = $@"
            SELECT
                ...
                CASE
                    ...
                    WHEN nd.VaiTro = 'ADMIN' THEN N'Admin'
                    ELSE N'Staff'
                END AS HoTen,
                kh.Email
            FROM NguoiDung nd
        ...
        if (!string.IsNullOrEmpty(role))
        {
            cmd.Parameters.AddWithValue("@VaiTro", role.ToUpperInvariant());
        }
        if (!string.IsNullOrEmpty(status))
        {
            cmd.Parameters.AddWithValue("@TrangThai", status.ToUpperInvariant());
        }
