...
namespace quanlynganhangtructuyen.Controllers
{
    ...
    public class TransactionController : BaseController
    {
        ...
        [HttpGet("lookup")]
        public async Task<IActionResult> TraCuuTaiKhoanNhan([FromQuery] string keyword)
        {
            ...
            try
            {
                var ketQua = await _transactionService.TraCuuTaiKhoanNhanAsync(keyword);
                return Ok(ketQua);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> XacNhanOTP([FromBody] XacNhanOTPRequest req)
        {
            ...
            try
            {
                var ketQua = await _transactionService.XacNhanOTPVaChuyenTienAsync(maNguoiDung.Value, req.MaGiaoDich, req.MaOTP);
                return Ok(ketQua);
            }
            catch (Exception ex)
            {
